...
signal_led_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (sscanf(buf, "%d %d %d %d", &rgb_index, &r_g_b, &color, &br) == 4) {
		if (rgb_index < 0 || rgb_index > (AW22XXX_RGB_MAX - 1)) {
			dev_err(aw22xxx->dev, "rgb_index error: 0 ~ 8");
			return len;
		}
		if (r_g_b < 0 || r_g_b > 2) {
			dev_err(aw22xxx->dev, "r_g_b error: 0 ~ 2");
			return len;
		}
		if (color < 0 || color > CURRENT_MAX) {
			dev_err(aw22xxx->dev, "color error: 0 ~ 0x%02X", CURRENT_MAX);
			return len;
		}
		if (br < 0 || br > PWM_MAX) {
			dev_err(aw22xxx->dev, "br error: 0 ~ 0x%02X", PWM_MAX);
			return len;
		}
		ret = aw22xxx_i2c_write(aw22xxx, REG_PAGE, 0);
		if (ret < 0) {
			dev_err(aw22xxx->dev, "i2c_write error");
			return ret;
		}
